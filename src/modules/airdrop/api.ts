...
export function fetchAirdropClaims(page = 1, limit = 10): Promise<PaginatedResult<APIAirdropClaims>> {
  return (
    GraphClient.get({
      query: gql`
        query($first: Int) {
      ...
      .then(result => {
        console.log(result);
        return {
          data: result.data.claims.slice(limit * (page - 1), limit * page),
          meta: { count: result.data.claims.length, block: page },
        };
      })
      // .then(({ data }) => {
      //   return {
